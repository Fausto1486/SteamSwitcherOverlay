...
| Backend | Status |
|---|---|
| DX9 | **Re-enabled, not yet confirmed.** `DX9::Install()` is now called unconditionally in `InstallWorkerThreadProc`, same as DX11 — but as of this change it has never actually been confirmed firing on a real DX9 game. Treat the first real test against a DX9 title as the actual verification step, not this re-enable. |
...
- **32-bit games: build config exists, unverified.** `SteamSwitcherOverlay.vcxproj` already has a full Win32 configuration (`TargetName=SteamSwitcherOverlay32`, correct output path, post-build copy to `Data\Mods32`), and `ModInjector.cs` already has full bitness-aware injection plumbing on the SteamSwitcher side (`IsWow64Process` detection, the `Mods32\` folder convention, `ModKitInjectHelper32.exe` for the cross-bitness `CreateRemoteThread`/`LoadLibraryA` case a same-process injector can't do directly). None of this has actually been built and run against a real 32-bit game yet, though — MinHook and ImGui claim to be architecture-agnostic and nothing in this project's own code has a `_WIN64`-gated or pointer-size-dependent path, but that's an unverified claim until a real Win32 build is tested end-to-end. Classic (pre-D3D9) titles — Direct3D versions before 9, and DirectDraw — have no support at all in any bitness; ImGui has no backend for either, so that tier needs an entirely separate, non-ImGui renderer hooked directly into the game's own present call.
