...
- **A `MONO` badge**, right-aligned in the "MODS - click to configure" strip, the moment `Overlay::g_runtimeInfo` is non-empty — set by `RUNTIMEKIND|<mono\|il2cpp>` over `OverlayPipe` (see [Pipe Protocol](#pipe-protocol)), SteamSwitcher's own process-level scan of the *target game's* loaded modules, sent the moment this DLL is injected. Deliberately **not** derived from anything ModKit reports (no shared-memory read, no dependency on `ModKit.dll` being present or any mod using `MonoKit.h`) — "is this game Mono/IL2CPP" is a fact about the game itself, and showing it shouldn't require modding infrastructure to be present at all. A MonoKit-based mod actually hooking something is a related but different fact, and stays visible only through the separate "MonoKit pool: ..." footer line (`MonoPoolInfoText`, driven by `SharedDataReader::TryReadMonoPoolInfo` — that one *is* correctly ModKit-coupled, since it's specifically reporting on ModKit's own class-pool activity).
...
| `RUNTIMEKIND\|<mono\|il2cpp>` | SteamSwitcher's own process-level Mono/IL2CPP scan result (`MonoRuntimeDetector.cs`, checking the *target game's* loaded modules for `mono_thread_attach`/`il2cpp_thread_attach` exports — the same signature-based method Cheat Engine's own MonoDataCollector uses, not a filename guess), sent the moment this DLL is injected. Sets `Overlay::g_runtimeInfo`, read every frame by `DrawStatusPanel`'s `MONO` badge check (see [Session Header, Dim Backdrop, and Logs Button](#session-header-dim-backdrop-and-logs-button)) — entirely independent of ModKit or any mod. An unrecognized value is logged and ignored rather than applied. |
